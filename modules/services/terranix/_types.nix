...
let
  ...
  # Shared submodule body so both the NixOS option and the terranix-class
  # container can type-check `.local` against the same declarations.
  localSubmodule = pkgs: submod: {
    options = {
      terraformWrapper = {
          ...
          options = {
            terraformConfiguration = mkOption {
              ...
              default = inputs.terranix.lib.terranixConfiguration {
                inherit pkgs;
                inherit (submod.config) extraArgs;
      ...
      extraArgs = mkOption {
        description = ''
          Extra arguments accessible from Terranix `configuration` modules
          (passed as `specialArgs` to `terranixConfiguration`).
        '';
...
in
{
  ...
  inherit mkLocalOption;

  localModule = pkgs: mkLocalOption pkgs { inherit pkgs; };
}
